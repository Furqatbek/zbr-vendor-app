...
export default function PaymentSettingsScreen() {
  ...
  const paymentMethods = [
    { id: '1', type: t('payments.bankAccount'), detail: 'Chase ****4829', icon: 'business-outline' as const, isDefault: true },
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      {/* Payment Methods */}
      <Text style={styles.sectionTitle}>{t('payments.paymentMethods')}</Text>
          ...
          <View style={styles.methodRow}>
            ...
            {method.isDefault && (
              <View style={styles.defaultBadge}>
                <Text style={styles.defaultText}>{t('payments.defaultLabel')}</Text>
              </View>
            )}
          </View>
        </Card>
      ...
      {/* Payout History */}
      <Text style={styles.sectionTitle}>{t('payments.recentPayouts')}</Text>
      ...
      {/* Tax Info */}
      <Text style={styles.sectionTitle}>{t('payments.taxInformation')}</Text>
      <Card style={styles.taxCard}>
        <View style={styles.taxRow}>
          <Ionicons name="document-text-outline" size={20} color={Colors.gray500} />
          <View style={styles.taxInfo}>
            <Text style={styles.taxLabel}>{t('payments.taxId')}</Text>
        ...
        <View style={[styles.taxRow, { borderTopWidth: 1, borderTopColor: Colors.gray100, paddingTop: Spacing.md, marginTop: Spacing.md }]}>
          ...
          <View style={styles.taxInfo}>
            <Text style={styles.taxLabel}>{t('payments.salesTaxRate')}</Text>
