export default {
  ...
  deleteAccount: {
    ...
    intro: 'Это навсегда удалит ваш партнёрский аккаунт ZBR и все связанные с ним данные.',
    introWithRestaurant: 'Это навсегда удалит ваш партнёрский аккаунт ZBR и уберёт {{restaurant}} с платформы.',
    whatHappens: 'Что произойдёт',
    consequenceOrders: 'Вы сразу перестанете получать заказы.',
    consequenceListing: 'Ваш ресторан исчезнет из приложения ZBR для клиентов.',
    consequenceAccess: 'Вы потеряете доступ к меню, отчётам и истории выплат.',
    consequenceRecords: 'Финансовые записи, которые мы обязаны хранить по закону, сохранятся, но не будут связаны с действующим аккаунтом.',
    irreversible: 'Это действие необратимо.',
    reasonLabel: 'Почему вы уходите? (необязательно)',
    reasonPlaceholder: 'Расскажите, что пошло не так...',
    confirmLabel: 'Введите {{email}} для подтверждения',
    deleteButton: 'Удалить мой аккаунт',
    finalTitle: 'Удалить аккаунт?',
    finalMessage: 'Ваш аккаунт и ресторан будут удалены навсегда. Это действие необратимо.',
    confirmDelete: 'Удалить',
    failedTitle: 'Не удалось удалить аккаунт',
    failedMessage: 'Что-то пошло не так. Попробуйте ещё раз или обратитесь в поддержку.',
    contactSupport: 'Связаться с поддержкой',
    emailSubject: 'Запрос на удаление аккаунта',
    helper: 'Удаление обрабатывается ZBR. Если по аккаунту остались незавершённые вопросы, поддержка может связаться с вами.',
  },
} as const;
